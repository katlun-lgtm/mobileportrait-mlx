...
class TPS:
    ...
    def __init__(self, mode, bs, **kwargs):
        ...
        if mode == "random":
            ...
            kp_2 = kwargs["kp_2"]
            device = kp_1.device
            # MPS portability: kp_1.type() returns the legacy string 'torch.mps.FloatTensor'
            # which `.type(...)` rejects on MPS. Use the dtype object — `.type(dtype)` is MPS-safe.
            kp_type = kp_1.dtype
...
def make_coordinate_grid(spatial_size, type):
    ...
    """
    h, w = spatial_size
    # MPS portability: the legacy `.type('torch.*.FloatTensor')` string idiom that TPS passes
    # (via kp.type()) is rejected on MPS ("invalid type: 'torch.mps.FloatTensor'"). It only
    # works on CPU/CUDA. Accept the legacy string but use float32; callers apply .to(device).
