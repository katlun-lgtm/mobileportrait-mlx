...
class DenseMotionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # MobilePortrait deltas (operate on the last hourglass feature = prediction[-1]).
        feat_ch = self.hourglass.out_channels[-1]
        # Δ2: residual optical flow (+2 channels) added to the composed deformation.
        self.residual_flow = nn.Conv2d(feat_ch, 2, kernel_size=(7, 7), padding=(3, 3))
        # Δ3: facial-knowledge mask heads (training only; consumed by GeneratorFullModel).
    ...
    def forward(
        ...
        feat = prediction[-1]  # Δ2/Δ3: last hourglass feature

        contribution_maps = self.maps(feat)
        if dropout_flag:
        ...
        # Δ2: add the residual optical flow (B,2,h,w) -> (B,h,w,2)
        deformation = deformation + self.residual_flow(feat).permute(0, 2, 3, 1)

        out_dict["deformation"] = deformation  # Optical Flow

        # Δ3: facial-knowledge mask predictions (training only; used by L1 losses)
        if self.training:
            out_dict["fg_mask_pred"] = torch.sigmoid(self.fg_mask_head(feat))
            out_dict["lmk_mask_pred"] = torch.sigmoid(self.lmk_mask_head(feat))
