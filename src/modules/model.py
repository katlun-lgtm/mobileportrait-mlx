...
from modules.util import AntiAliasInterpolation2d, TPS, kp2gaussian
from torchvision import models
import numpy as np


def landmark_mask_from_points(points, size, kp_variance=0.01):
    """Rasterise (B,N,2) normalised landmarks ([-1,1]) into a (B,1,size,size) soft mask.

    Splats a gaussian per landmark (TPS kp2gaussian) and per-pixel max across landmarks —
    a smooth 0..1 landmark heatmap usable as the Δ3 landmark-mask L1 target.
    """
    g = kp2gaussian(points, spatial_size=(size, size), kp_variance=kp_variance)
    return g.max(dim=1, keepdim=True).values
